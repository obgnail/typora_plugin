...
- [插件列表](#插件列表)
- [Q&A](#qa)
- [如何使用：Windows/Linux 平台](#如何使用windowslinux-平台)
- [如何使用：Archlinux 平台](#如何使用archlinux-平台)
- [插件使用说明](#插件使用说明)
- [编辑增强](#编辑增强)
- [视图与主题](#视图与主题)
- [组件渲染](#组件渲染)
...
> 格式说明：插件名(fixedName) — 功能描述，`[×]` = 默认关闭（需手动开启），留空 = 默认启用。fixedName 为 settings.user.toml 中配置该插件的标识名。
...
└── 高级功能
    ├── hotkeys — 快捷键注册中心（高级）
    ├── action_buttons — 于右下角添加功能按钮（高级）
    ├── custom — 开放平台，用户自定义插件（高级）
    ├── remote_control — 外部操控 Typora（高级）[×]
    ├── cipher — 加密文件 [×]
...
- [hotkeys](#hotkeys) — 快捷键注册中心（高级）
- [action_buttons](#action_buttons) — 于右下角添加功能按钮（高级）
- [custom](#custom) — 开放平台，用户自定义插件（高级）
- [remote_control](#remote_control) — 外部操控 Typora（高级）×
- [cipher](#cipher) — 加密文件 ×
