...
class MarkdownlintPlugin extends BasePlugin {
  ...
  style = () => `
#plugin-markdownlint {
  --markdownlint-width: 580px;
  top: 8%;
  left: calc(100% - var(--markdownlint-width) - 30px);
  width: var(--markdownlint-width);
  z-index: 9993;
}
...
#plugin-markdownlint-indicator {
  ...
  background-color: ${this.config.BUTTON_PASS_COLOR};
  opacity: ${this.config.BUTTON_OPACITY};
  margin: 7px;
  cursor: pointer;
  transition: background-color 0.1s ease 0s;
  z-index: 9992;
}
#plugin-markdownlint-indicator[lint-check-failed] {
  background-color: ${this.config.BUTTON_ERROR_COLOR};
}`
    ...
    this.linter = {
      configure: async ({ ruleConfig = this.config.RULE_CONFIG, customRuleFiles = this.config.CUSTOM_RULE_FILES, persistent = false } = {}) => {
        ...
        client.configure({
          ...
          polyfillLibs: ["plugin/global/core/polyfill.js", "plugin/markdownlint/polyfill.js"].map(f => this.utils.joinPluginPath(f)),
