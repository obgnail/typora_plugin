...
cp -f "$WINDOW_HTML_PATH" "$WINDOW_HTML_BAK_PATH" || panic "Failed to create backup of window.html."
...
if [[ -f "$SETTINGS_DIR/settings.user.toml" ]]; then
  chmod 0777 "$SETTINGS_DIR/settings.user.toml" || panic "Failed to set permissions for settings.user.toml."
else
  echo "      -> settings.user.toml not found. Skipping permission set for it."
fi
