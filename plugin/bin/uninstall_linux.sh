...
readonly ROOT_DIR="$(cd "$SCRIPT_DIR/../.." 2>/dev/null && pwd -P)"
if [[ -z "$ROOT_DIR" ]]; then
  panic "Execution context error. Please manually move the 'plugin' folder to Typora's installation directory before running this script."
fi
...
if ! grep -qF "$PLUGIN_SCRIPT" "$WINDOW_HTML_PATH"; then
  echo -e "\033[0;32mPlugin has already been uninstalled.\033[0m"
  exit 0
fi

echo "[7/8] Delete window.html.bak"
rm -f "$WINDOW_HTML_BAK_PATH" || echo -e "\033[0;33m[WARN] Could not remove '$WINDOW_HTML_BAK_PATH'. File might not exist or permission issue.\033[0m"
...
echo -e "\033[0;32m\nPlugin uninstalled successfully! Please restart Typora.\033[0m"
