class helpPlugin extends BasePlugin {
    ...
    donate = () => {
        ...
        const qrcode = () => {
            ...
            const qr = [
                {name: "wechat", length: 41, color: "#1AAD19", hex: wechat},
                {name: "ali", length: 45, color: "#027AFF", hex: ali},
            ]
            const width = (qrMaxSize + qrMargin) * qr.length - qrMargin;
            const id = this.utils.randomString();
            const adaptDPR = (canvas, ctx) => {
            ...
            const after = () => {
                ...
                qr.forEach(({hex, length, color}) => {
                    const bin = hex.split("-").map(h => parseInt(h, 16).toString(2).padStart(length, "0")).join("");
                    const table = bin.match(new RegExp(`(.{1,${length}})`, "g"));
                    const size = qrMaxSize / length;
                    for (let rowIdx = 0; rowIdx < table.length; rowIdx++) {
                        for (let colIdx = 0; colIdx < table[0].length; colIdx++) {
                            ctx.fillStyle = (table[rowIdx][colIdx] === "1") ? color : "#fff";
                            ctx.fillRect(rowIdx * size, colIdx * size, size, size);
                        }
                    }
                    ctx.translate(qrMaxSize + qrMargin, 0);
                })
            }
            this.utils.loopDetector(() => document.getElementById(id), after);
            return `<canvas id="${id}" width="${width}" height="${qrMaxSize}" style="margin: auto;display: block;"></canvas>`
        }

        const components = [{label: qrcode(), type: "p"}];
        this.utils.modal({title: "请开发者喝咖啡", components}, console.debug);
    }
