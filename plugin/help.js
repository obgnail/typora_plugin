class helpPlugin extends BasePlugin {
    ...
    donate = () => {
        ...
        const qrcode = () => {
            ...
            const qr = [{color: "#1AAD19", hex: wechat}, {color: "#027AFF", hex: ali}];
            const width = (qrSize + qrMargin) * qr.length - qrMargin;
                ...
                for (const {hex, color} of qr) {
                    ...
                    for (let colIdx = 0; colIdx < table.length; colIdx++) {
                        for (let rowIdx = 0; rowIdx < table[0].length; rowIdx++) {
                            ctx.fillStyle = (table[colIdx][rowIdx] === "1") ? color : "#fff";
            ...
            return `<canvas id="${id}" width="${width}" height="${qrSize}" style="margin: auto;display: block;"></canvas>`
        }
