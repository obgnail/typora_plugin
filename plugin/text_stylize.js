class textStylizePlugin extends BasePlugin {
    ...
    html = () => {
        const toolMap = {
            ...
            borderColor: ["边框", `<svg xmlns="http://www.w3.org/2000/svg" height="22" viewBox="0 -960 960 960" width="22"><path d="M80 0v-800V0H80Zm160-320h56l312-311-29-29-28-28-311 312v56Zm-80 80v-170l448-447q11-11 25.5-17t30.5-6q16 0 31 6t27 18l55 56q12 11 17.5 26t5.5 31q0 15-5.5 29.5T777-687L330-240H160Zm560-504-56-56 56 56ZM608-631l-29-29-28-28 57 57Z"/><path class="color-indicator" d="M80 0v-160h800V0"/></svg>`],
        ...
        const trs = this.utils.chunk(COLORS, NUM_PER_LINE).map(colorList => {
            const tds = colorList.map(color => `<td style="background-color: ${color}" color="${color}"></td>`)
            return `<tr>${tds.join("")}</tr>`
        })

        return `
            <div id="plugin-text-stylize" class="plugin-common-modal plugin-common-hidden">
                <div class="stylize-tool">${tools.join("")}</div>       
                <table class="stylize-palette"><tbody>${trs.join("")}</tbody></table>     
            </div>
        `
    }
