class EasyModifyPlugin extends BasePlugin {
  ...
  changeHeadersLevel = incr => {
    ...
    _getTargetHeaders().forEach(node => {
  ...
  trailingWhiteSpace = async () => {
    if (this._showWarnDialog) {
      const { response, checkboxChecked } = await this.utils.showMessageBox({
        type: "warning",
        message: this.i18n.t("act.trailing_white_space.hint"),
    ...
    const replaceFlag = 2
    const tailSpace = "  "
    this.utils.entities.querySelectorAllInWrite("p[cid]").forEach(el => {
  ...
  insertMindmap = (type, target) => {
    ...
    const fn = mermaidFn[type]
    if (!fn) return

    const tree = this.utils.getTocTree()
    const tokens = fn(tree)
