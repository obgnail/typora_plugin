class Highlighter {
  ...
  process = () => {
    ...
    document.querySelector(".plugin-search-highlights").addEventListener("mousedown", ev => {
      ...
      if (!target) return
      const className = [...target.classList.values()].find(c => c.startsWith("cm-plugin-highlight-hit"))
      if (!className) return
  ...
  _highlightMarker = marker => {
    document.querySelectorAll(".plugin-highlight-outline").forEach(el => el.classList.remove("plugin-highlight-outline"))
