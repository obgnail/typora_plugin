...
- **灵活的生命周期：** 提供 `prepare`、`init`、`process`、`finalize`、`callback` 等生命周期函数，允许在插件不同阶段执行自定义逻辑。
...
// 5. style: 用于插入 style 标签。
// 6. html: 用于插入 HTML 标签。返回 Element 类型或表示元素的字符串。
// 7. hotkey: 为 callback 函数注册快捷键。返回一个字符串数组，或者 [{ hotkey: "", callback: () => console.log(123) }] 格式的对象数组
// 8. prepare: 最先执行的函数，用于初始化插件所需的数据或进行前置检查。如果返回 utils.PLUGIN_LOAD_ABORT，则停止加载插件
// 9. process: 在这里添加添加插件业务逻辑
// 10. callback: 当用户点击右键菜单选项或键入快捷键时调用的回调函数。anchorNode 参数表示鼠标光标所在的 Element
// 11. 将插件类导出为 plugin
```
