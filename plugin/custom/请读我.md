...
//    - utils：插件系统自带的静态工具类，其定义在 `./plugin/global/core/plugin.js/utils`。其中有三个重要的函数：utils.getPlugin(fixedName) 和 utils.getCustomPlugin(fixedName) 用于获取已经实现的全部插件，调用其 API，具体的 API 可看 ./plugin/custom/openPlatformAPI.md 文件。utils.eventHub.addEventListener(eventType, listener) 用于监听 Typora 的生命周期事件。
