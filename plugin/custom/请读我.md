...
> 这个插件会在调用时，弹出提示框，显示 `this is hello world plugin`。



完成以上步骤后，重启 Typora，鼠标右键弹出菜单，选择 `常用插件 -> 自定义插件 -> 你好世界`。
...
1. 在右键菜单中添加一个 `获取标题路径` （类似于 `messing9.md\无 一级标题\开放平台 二级标题\window_tab 三级标题`）的功能。
2. 当光标位于【正文标题】中才可使用。
3. 快捷键 `ctrl+shift+y`。
