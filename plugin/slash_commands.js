...
const getOrderStrategy = (type) => {
  ...
  const predefined = (commands) => commands
...
class SlashCommandsPlugin extends BasePlugin {
  ...
  matchStrategy = getMatchStrategy(this.config.MATCH_STRATEGY)
  orderStrategy = getOrderStrategy(this.config.ORDER_STRATEGY)
  commands = new Map(
    this.config.COMMANDS
      .filter(c => c.enable && c.keyword && /[A-Za-z0-9]+/.test(c.keyword))
      .map(c => [
        c.keyword.toLowerCase(),
        {
          ...c,
          scope: c.scope || this.SCOPE.PLAIN,
          icon: c.icon || (c.type === this.TYPE.COMMAND ? "🧰" : "🧩"),
          cursorOffset: c.cursorOffset || defaultOffset,
          hint: this.utils.escape(c.hint || ""),
        },
      ]),
  )
  matched = new Map()
  handler = {
    search: input => this.orderStrategy([...this.matched.keys()], input),
    render: (suggest, isActive) => {
  ...
  prepare = () => this.commands.size ? undefined : this.utils.PLUGIN_LOAD_ABORT
