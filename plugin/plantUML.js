class PlantUMLPlugin extends BasePlugin {
  ...
  create = async ($wrap, content) => {
    ...
    $wrap[0].innerHTML = contentType.startsWith("image/svg+xml")
      ? buffer.toString() : contentType.startsWith("text/plain")
        ? `<pre>${buffer.toString()}</pre>` : contentType.startsWith("image/png")
          ? `<img src="data:image/png;base64,${buffer.toString("base64")}">` : new Error(`No Format Matched: ${contentType}`)
  }
